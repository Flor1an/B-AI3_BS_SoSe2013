...
public class PageTable {
   ...
   * aktuellen Index, mit Referenced-Bit = 0 (false) ausw�hlen Sonst R-Bit aufl
  ...
  private PageTableEntry randomAlgorithm(PageTableEntry newPte) {
    // ToDo
	  Random r = new Random();
	  int randIndex = r.nextInt(pteRAMlist.size());
	  PageTableEntry oldPte = pteRAMlist.get(randIndex);
	  
	  
	  pteRAMlist.remove(randIndex);
	  pteRAMlist.add(randIndex, newPte);
